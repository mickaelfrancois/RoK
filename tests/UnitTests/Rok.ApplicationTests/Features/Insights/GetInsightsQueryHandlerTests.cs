...
namespace Rok.ApplicationTests.Features.Insights;
...
public class GetInsightsQueryHandlerTests
{
    ...
    public async Task Handle_ShouldDelegateToRepository_AndReturnItsInsights()
    {
        ...
        GetInsightsQueryHandler handler = new(repository.Object);

        // Act
        InsightsDto result = await handler.HandleAsync(new GetInsightsQuery { Month = month }, CancellationToken.None);
