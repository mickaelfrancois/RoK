...
namespace Rok.ApplicationTests.Features.ListeningEvents;
...
public class CreateListeningEventCommandHandlerTests
{
    ...
    public async Task Handle_ShouldIgnoreEventAsFastChange_WhenPlayedUnder30SecondsAndUnder20Percent()
    {
        ...
        CreateListeningEventCommandHandler handler = new(repository.Object);
        CreateListeningEventCommand command = new() { TrackId = 1, DurationPlayed = 10, DurationTotal = 200 };

        // Act
        Result<long> result = await handler.HandleAsync(command, CancellationToken.None);
    ...
    public async Task Handle_ShouldPersistEvent_AndMarkItAsSkipped_WhenUnder20PercentButPlayedMoreThan30Seconds()
    {
        ...
        repository.Setup(r => r.AddAsync(It.IsAny<ListeningEventEntity>(), It.IsAny<RepositoryConnectionKind>()))
            ...
            .ReturnsAsync(88);
        CreateListeningEventCommandHandler handler = new(repository.Object);
        CreateListeningEventCommand command = new() { TrackId = 1, AlbumId = 2, ArtistId = 3, GenreId = 4, DurationPlayed = 40, DurationTotal = 500 };
    ...
    public async Task Handle_ShouldPersistEventWithoutSkipFlag_WhenCompletionRateReaches20Percent()
    {
        ...
        repository.Setup(r => r.AddAsync(It.IsAny<ListeningEventEntity>(), It.IsAny<RepositoryConnectionKind>()))
            ...
            .ReturnsAsync(7);
        CreateListeningEventCommandHandler handler = new(repository.Object);
        CreateListeningEventCommand command = new() { TrackId = 1, DurationPlayed = 60, DurationTotal = 200 };
    ...
    public async Task Handle_ShouldReturnFailure_WhenRepositoryReturnsNonPositiveId()
    {
        ...
        CreateListeningEventCommandHandler handler = new(repository.Object);
        CreateListeningEventCommand command = new() { TrackId = 1, DurationPlayed = 100, DurationTotal = 200 };
